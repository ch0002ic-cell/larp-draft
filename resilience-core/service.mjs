...
const operations = new Set([
  ...
  'ingestText', 'ingestDocument', 'discover',
...
export function createLedgerService({ ledger, verifySession, resolveAccess, now = Date.now }) {
  ...
  const resolveActor = async (credential) => {
    let actor;
    try {
  ...
  return Object.freeze({
    async execute(credential, operation, ...args) {
      ...
      let actor = await resolveActor(credential);
      if (operation === 'ingestDocument') {
        const [input] = inputs;
        ledger.authorizeIngestion(actor, input.matterId);
        const extracted = await extractDocument(input.bytes, input);
        const refreshed = await resolveActor(credential);
        if (refreshed.tenantId !== actor.tenantId || refreshed.userId !== actor.userId) denied();
        actor = refreshed;
        if (input.remote) extracted.provenance.remote = structuredClone(input.remote);
        return ledger.addArtefact(actor, { ...input, ...extracted });
      }
