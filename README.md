...
```bash
node --test resilience-core/ledger.test.mjs
...
Actors are **trusted inputs** from a future authentication/permission adapter, not credentials or a safe HTTP request schema. The caller must obtain current tenant, user, roles and matter permissions from that adapter. A local database administrator can bypass the SQL immutability triggers; this is neither cryptographic tamper proofing nor encrypted storage. Protect the database and its WAL/backups, and use synthetic data until a deployment is approved.
...
Validation: **34 focused tests pass**, covering evidence failures, no-impact approval gates, scope and lifecycle separation, assessment review/invalidation, access boundaries, cache invalidation inputs, revision conflicts, restart persistence, future-schema rejection, corrected evidence and executed-document routing. The synthetic demo also completed. App lint/build were not run for this standalone module because application code and dependencies were unchanged and Bun/node_modules were unavailable. Next integration work should begin with a trusted identity adapter and a legally reviewed assertion pack, followed by measured retrieval and evaluation of the manually reviewed assessment workflow.
