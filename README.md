...
| `ingestDocument` (service) | Extract plain text, PDF, DOCX or opt-in PNG OCR in the generic service, rechecking identity and permissions after asynchronous parsing. Microsoft uses its verified SharePoint import path. |
...
PDF parsing uses pinned [PDF.js](https://mozilla.github.io/pdf.js/api/draft/module-pdfjsLib.html) to extract text page by page. Empty/scan-like pages remain failed unless opt-in local OCR returns text; review gaps remain in either case. A retained review gap covers unexamined visual layout, graphics, forms, annotations and embedded attachments. DOCX parsing uses [yauzl](https://github.com/thejoshwolfe/yauzl) and [saxes](https://github.com/lddubeau/saxes) to read UTF-8 Word XML, including body/table text, standard-named headers, footers, footnotes, endnotes and comments. It preserves both inserted/deleted wording where present and flags unresolved revision meaning, fields, hidden text, unsupported parts and external relationships. It does not follow links or resolve XML entities/DTDs.

Both rich formats deliberately remain `inventoryComplete: false` pending review of non-text content and layout. Text extraction is useful evidence, but it does not clear a document or authorize binary publication. Custom Word-part mappings, UTF-16 XML, visual table reconstruction and Word tracked-change editing remain unimplemented. The existing plain-text publication restrictions still apply.

Parsing runs in a worker with a 10-second deadline (30 seconds when OCR is enabled), a 128 MiB JavaScript old-generation limit and a 4 MiB stack limit. Raw inputs are capped at 1 MiB; DOCX at 256 package entries and 8 MiB declared expansion; extracted output at 2 MiB; PDF processing at 200 pages. Unprocessed page counts and parser failures stay visible. Duplicate/traversing ZIP paths, encrypted entries, invalid parsed-part CRCs and DTDs are rejected. Worker limits do not bound all native allocations or constitute an OS security sandbox; production parser isolation still needs deployment validation. Parser diagnostics are drained without copying document text into application logs.

Set `ocr: true` in `ingestDocument` or SharePoint `import` input to enable local English [Tesseract.js](https://github.com/naptha/tesseract.js/blob/master/docs/local-installation.md) transcription for PNG images and PDF pages with no extracted text. The pinned English model is installed with the module; runtime OCR uses local files, without CDN downloads or language-cache writes. OCR is capped at three images/pages and five million pixels per image, within the existing input/output and worker limits. PDF pages are rendered at scale 2; oversized pages fail visibly. Mixed pages with an existing text layer do not receive OCR, so text embedded in their images remains a review gap. JPEG/TIFF input and embedded Word images are not processed.

OCR provenance records the engine version, compressed model hash, image dimensions and per-segment recognition confidence. Confidence measures transcription only, not legal accuracy. Blank, malformed, oversized or timed-out input stays failed, and all OCR imports remain `inventoryComplete: false`; neither no-impact clearance nor publication becomes available from OCR alone. Synthetic transcription tests establish mechanics only. The unchanged evaluation corpus still contains its original unparsed-scan abstention and is not automatically reprocessed by this feature.
...
### Separate local review console

Run `npm run review --prefix resilience-core` and open **http://127.0.0.1:4173**. The [console](resilience-core/review-server.mjs) starts an explicitly labeled synthetic demonstration in memory. It shows proposed change sets, source text, interpretations, applicability/exceptions, assessment rationale, current/proposed wording, legal decisions, owner decisions and publication status. Decisions require reasons and expected revisions. Publication and reconciliation use the existing worker gates; restarting the demo resets its synthetic data. This interface leaves the existing hackathon app untouched.

For an approved Microsoft environment, run `node resilience-core/review-server.mjs --microsoft` with the same local configuration, database, secret and short-lived token used by the CLI. **This mode can write to SharePoint.** Every operation acts as that token holder and rechecks Microsoft access; credentials are never sent to the browser. The console does not implement browser sign-in, token renewal or simultaneous identities. An owner with a different identity must run their own authorized session against the approved shared ledger. Prepare imports, assertions, assessments and change-set proposals through the CLI/service; the console currently handles review and publication of existing proposals.

The server binds only to IPv4 loopback, validates the exact Host/Origin, requires a random per-start session token for API calls, serves only named static assets, forbids framing, sends no-store responses and renders evidence as text. It accepts one API operation at a time with a 16 KiB request limit. This is a trusted local operator tool, not a remotely deployable multi-user service; other processes/users with access to the same local session remain within its trust boundary. Microsoft mode has not been tested against a live tenant.

Browser validation uses Chromium: `npx --prefix resilience-core playwright install chromium`, then `npm run test:browser --prefix resilience-core`. CI also runs this synthetic workflow, including reason validation, evidence inspection, separate approvals, partial/final publication and mobile-width overflow checks. These checks do not establish full accessibility compliance or live Microsoft usability.
...
| Broader discovery and extraction | Validate the implemented PDF/DOCX, local English OCR and citation/proximity baselines on representative permitted files; add richer layout handling and evaluated semantic discovery. Select any model/deployment after the partner's data-handling decision; no hosted model calls or embedding pipeline have been added. |
...
| Product integration | Validate the separate local review console with users; add browser sign-in, identity switching, import/assessment authoring and accessible claims-challenge handling for a deployed service. The existing hackathon web app remains unchanged. |
